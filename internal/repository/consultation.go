...
func (r *repo) CreatePatientRequest(input *models.ConsultationRequest) int {
	var id int
	query := "INSERT INTO request (user_id, description, created_at) VALUES($1, $2, $3) RETURNING id"
...
func (r *repo) GetRecommendation(id int) ([]models.DoctorRecomemndation, error) {
	...
	query := "SELECT req_id, recomm FROM rec recommendation JOIN r request ON rec.req_id = r.id where r.user_id = $1"
	...
	for rows.Next() {
		...
		err := rows.Scan(&recom.Id, &recom.Recommendation)
		if err != nil {
...
func (r *repo) GetPatientRequest(id int) (*models.ConsultationRequest, error) {
	...
	query := "SELECT user_id, description, created_at FROM request WHERE id=$1"
...
func (r *repo) CreateRecommendation(input *models.DoctorRecomemndation) int {
	var id int
	query := "INSERT INTO recommendation (req_id, recomm) VALUES($1, $2) RETURNING id"
